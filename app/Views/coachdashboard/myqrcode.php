...
<script>
    function generateQRCode(coachID) {
        if (!coachID || coachID === "null") {
            console.error("❌ CoachID is missing or null.");
            return;
        }

        console.log("✅ Generating QR for:", coachID); // Debugging
    ...
    window.onload = function () {
        const coachID = <?= json_encode(session()->get('CoachID')); ?>; // Get CustomerID from session
